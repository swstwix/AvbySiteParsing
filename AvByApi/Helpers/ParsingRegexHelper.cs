...
namespace WpfApplication1.Static
{
    public class ParsingRegexHelper
    {
        ...
        public static IDictionary<string, int> AllBrand(string html)
        {
            IDictionary<string,int> brands = new Dictionary<string, int>();

            string pagePart = Regex.Match(html, @"<OPTION value=""1"">Alfa[\s\S]*Урал</OPTION>").ToString();
        ...
        public static IDictionary<string, int> AllModels(string html)
        {
            IDictionary<string, int> models = new Dictionary<string, int>();
            string pagePart = Regex.Match(html, @"Модель:</td>[\W]*[\s\S]*Ещё транспорт для поиска?").ToString();
            const string template = @"<OPTION value=""(?<value>[\d]+?)"">(?<key>[\s\S]+?)</OPTION>";
            MatchCollection matches = Regex.Matches(pagePart, template);
